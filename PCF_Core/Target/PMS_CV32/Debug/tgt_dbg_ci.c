    ...
    if (s_check == 0) {
        if (ci_abs(g_cycle_valid_comparison - g_mes_comp_value_timer) > 80) // TBD: 500 value
        {
            ...
            printf("Timer issue @ %ldus, interval: %d/%d\n\r", cycle_measure,
                   g_cycle_valid_comparison, g_mes_comp_value_timer);
#endif
            s_check = 3;
            correct = 0;
...
void vDebugCiInit(void) {
    ...
    g_mes_comp_value_systick =
        (uint32_t)((float)l_systick_us * ((float)DEFAULT_SYSTEM_CLOCK / (float)1000000.0));

    if ((l_systick_us % (int)g_TaskConfigTable.tap_period_us) != 0) l_tap_not_exact = 1;
    ...
    for (varFor i = 0; i < MAX_NUM_TASKS; i++) ci_test_tasks_exec[i] = 0;
}
        ...
        if (s_check == 0) {
            if (ci_abs(g_cycle_valid_comparison - g_mes_comp_value_systick) > 80) // TBD: 500 value
            {
                ...
                printf("Systick issue @ %ldus, interval: %d/%d\n\r", cycle_measure,
                       g_cycle_valid_comparison, g_mes_comp_value_systick);
#endif
                s_check = 3;
                correct = 0;
                // TBC: vTargetPcfExit(1);
...
void vDebugCiPrintInformation(void) {
    ...
    for (varFor task = 0; task < NUM_PERIODIC_TASKS; task++) {
        if (!l_tap_not_exact)
            printf("%s iterations:		%d / %d ", pcTaskGetName(taskHandles[task]),
                   ci_test_tasks_exec[task],
                   (l_systick_mul / g_TaskConfigTable.tasks_tap_multiplier[task]));
        else
            printf("%s iterations:		%d / %d ", pcTaskGetName(taskHandles[task]),
                   ci_test_tasks_exec[task],
                   (l_systick_mul / g_TaskConfigTable.tasks_tap_multiplier[task]) - 1);

        printf("	(%d us, %d Hz)\n\r",
               (g_TaskConfigTable.tasks_tap_multiplier[task] * g_TaskConfigTable.tap_period_us),
               (1000000 /
                (g_TaskConfigTable.tasks_tap_multiplier[task] * g_TaskConfigTable.tap_period_us)));
    }
#endif
