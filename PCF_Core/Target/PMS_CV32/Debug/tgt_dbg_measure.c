...
// #define REG_WBRANCH_Pos               0U                                            /*!< REG:
//  WBRANCH Position */
// #define REG_WBRANCH_Msk               (0x1UL << REG_WBRANCH_Pos)                   /*!< REG:
//  WBRANCH Mask */

// #define REG_WBRANCH_CYC_Pos           0U                                            /*!< REG:
//  WBRANCH_CYC Position */
// #define REG_WBRANCH_CYC_Msk           (0x1UL << REG_WBRANCH_CYC_Pos)               /*!< REG:
//  WBRANCH_CYC Mask */
...
// #define REG_DELAY_SLOT_Pos            11U                                           /*!< REG:
//  DELAY_SLOT Position */
// #define REG_DELAY_SLOT_Msk            (0x1UL << REG_DELAY_SLOT_Pos)                /*!< REG:
//  DELAY_SLOT Mask */

// #define REG_LD_EXT_Pos                12U                                           /*!< REG:
//  LD_EXT Position */
// #define REG_LD_EXT_Msk                (0x1UL << REG_LD_EXT_Pos)                    /*!< REG:
//  LD_EXT Mask */

// #define REG_ST_EXT_Pos                13U                                           /*!< REG:
//  ST_EXT Position */
// #define REG_ST_EXT_Msk                (0x1UL << REG_ST_EXT_Pos)                    /*!< REG:
//  ST_EXT Mask */

// #define REG_LD_EXT_CYC_Pos            14U                                           /*!< REG:
//  LD_EXT_CYC Position */
// #define REG_LD_EXT_CYC_Msk            (0x1UL << REG_LD_EXT_CYC_Pos)                /*!< REG:
//  LD_EXT_CYC Mask */

// #define REG_ST_EXT_CYC_Pos            15U                                           /*!< REG:
//  ST_EXT_CYC Position */
// #define REG_ST_EXT_CYC_Msk            (0x1UL << REG_ST_EXT_CYC_Pos)                /*!< REG:
//  ST_EXT_CYC Mask */

// #define REG_TCDM_CONT_Pos             16U                                           /*!< REG:
//  TCDM_CONT Position */
// #define REG_TCDM_CONT_Msk             (0x1UL << REG_TCDM_CONT_Pos)                 /*!< REG:
//  TCDM_CONT Mask */

// #define REG_EVENTS_NUM                17U                                           /*!< REG: All
//  events number */
...
{
    uint32_t result;
    // This function does not work because compiler cannot "convert" the csr_num to a 12-bit
    // Immediate.
...
/* Other Functions */
#if (MEASURE_ACTIVE == 1)
void vDebugMeasureIterationCount(void) {

    /* From ISR */

    static int dIterationCount = 0;
...
void vDebugMeasurePrintResults(void) {
#if (defined(PRINTF_ACTIVE) && defined(HRO_PRINTF))
    printf("End Measure execution, %d\n\r", lMeasureReadCsr(MEASURE_ZEROING));
    printf("Done\n\r");
#endif

    timerBuffer[Timerindex++] = (Timer_Data_t){'Z', 0};

    for (int p = 0; p < (MEASURE_N_OUTPUTS * MEASURE_N_ITERATION + 7); p++) {
        if (timerBuffer[p].where != '0') {
#if (defined(PRINTF_ACTIVE) && defined(HRO_PRINTF))
            if (timerBuffer[p].where == 'I') printf("\n\r");

            printf("Timer: %d, Where: %c, value: %d\n\r", p, timerBuffer[p].where,
                   timerBuffer[p].cycle);
#endif
        }

        if (timerBuffer[p].where == 'Z') break;
    }
#if (defined(PRINTF_ACTIVE) && defined(HRO_PRINTF))
    ...
    for (varFor task = 0; task < MAX_NUM_TASKS; task++) {
        printf("    %s:     %d\n\r", pcTaskGetName(taskHandles[task]),
               (int)uxTaskGetStackHighWaterMark(taskHandles[task]));
    }
#endif // printf
    vTargetPcfExit(0);
#endif // ndef ci_test
