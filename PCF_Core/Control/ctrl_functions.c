...
/*** Internal Functions ***/
varValue fCheckFrequencies(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
varValue fMultiInputProcess(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
...
varValue fComputeEstimatedTotalPower(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
varValue fCorePowerComputation(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
varValue fVoltageCastVote(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
varValue fVoltageChoice(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
varValue fFVpreAdaptation(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
inline void vFVdomainAdaptation(struct ctrl_task_index* tptr);
varValue fDummyPowerReduction(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
inline void vAlphaPowerReduction(varValue i_delta_power, varValue est_total_pw, struct ctrl_task_index* tptr);
varValue fComputeAlpha(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
inline void vComputeAlphaBindings(struct ctrl_task_index* tptr);
varValue fNormalizeApplyAlpha(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
//varValue fAlphaDomain(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
varValue fComputeTemperatureReduction(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
inline void vComputeFrequency(struct ctrl_task_index* tptr);
varValue fFrequencyCouplingTransformation(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
//This function has the same structure, but it is not called with the macro
varFor lPopulateCouplingStructure(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
//no inline is ok!
void vProcessFrequency(varFor i_core, varValue i_frequency, struct ctrl_task_index* tptr);
varValue fFrequency2PowerConversion(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args);
...
void MACRO_CP_CLUSTER_PARALLELIZE(struct function_param* param) {  
	...
	if ( (param->parall_num>1)&&(exec_gain<param->exec_time) ) {
		printf("PLplPlPl\n\r");
		if (param->return_value == NULL){bTargetClusterFork(vDummyFork, (void*)param, param->parall_num);}
		else {varValue accum[MAX_NUM_CLUSTER_CORE] = {0}; varValue *ptr_hold = param->return_value; param->return_value = (void*)accum;
		bTargetClusterFork(vDummyFork, (void*)param, param->parall_num);
		*ptr_hold = VD_ZERO; for(varFor ccore=0; ccore < param->parall_num; ccore++) {*ptr_hold += accum[ccore];}} }
	else {if (param->return_value == NULL) {param->f(0, param->total_iterations, 1, param->igl, param->ptr_args);}
			else{*(param->return_value) = param->f(0, param->total_iterations, 1, param->igl, param->ptr_args);}; }
}
...
void vConfigValueCtrlInit(ctrl_values_table_t *values)
{
	...
	{
		values->op.store_max_freq[domain]				= 
			g_config_sys.voltage_table[(0)*3 + 2];
		values->op.computed_domain_voltage[domain]	= 
			g_config_sys.voltage_table[(0)*3 + 0];
	}
	...
	*/
	...
	//if (bWriteGlobalVariable(&c_values_table, G_CTRL_PARAMETER_TABLE, GLOBAL_WRITE_ALL, sizeof(c_values_table), c_task_id) != PCF_TRUE)
	//{
		//TODO
...
void vMainTaskControlAlgorithm (task_param_t * tp)
{
	...
	ctrl_values_table_t l_values_ctrl; //TBC: I cannot avoid the case "I have the Cluster, but running in the FC"
	ctrl_inputs_t l_input; //TBC: I cannot avoid the case "I have the Cluster, but running in the FC"
	//TODO: REMOVE
	...
	if (parallel_num > 0) //If we are on the cluster //TBC: if there exist a better function
	{
		...
		unsigned int dma_id_value = 
			pulp_cl_idma_memcpy(&l_values_ctrl,
								tp->cl_ptr,
								sizeof(ctrl_values_table_t));

		unsigned int dma_id_input = 
			pulp_cl_idma_memcpy(&l_input,
								&g_input_ctrl,
								sizeof(ctrl_inputs_t));

		//TODO: there must be a better way to do this.
		//	A) These values are almost constant, so I need to create a "global cluster variable"
		//	B) If I want to change these values, I create a function to do so, and this function will also copy the changed 
		...
		unsigned int remove_cfg_ctrl = 
			pulp_cl_idma_memcpy(&cl_config_ctrl,
								&g_ControlConfigTable,
								sizeof(ctrl_config_table_t));

		unsigned int remove_cfg_sys = 
			pulp_cl_idma_memcpy(&cl_config_sys,
								&g_config_sys,
								sizeof(config_sys_t));
	...
	*/
	...
	if (p_values_ctrl->ctrl_counter >= 10*p_config_ctrl->therm.fuzzy_mul*p_config_ctrl->therm.der_mul) //TODO: mcm, and 10
	{
		p_values_ctrl->ctrl_counter = 2*p_config_ctrl->therm.fuzzy_mul*p_config_ctrl->therm.der_mul;
	}
	...
	*/
	...
	#if (!defined(USE_CTRL_STRCT_INVERSE) && !defined(USE_CTRL_FUZZY)) //(like this becasue both VB and Direct Needs it)
	p.f = fComputeTemperatureReduction;
	p.total_iterations = p_config_sys->num_cores;
	p.exec_time = 1;
	p.ptr_args = NULL;
	...
	if (parallel_num > 0) //If we are on the cluster //TBC: if there exist a better function
	{
		...
		unsigned int dma_id_value3 = 
			pulp_cl_idma_memcpy(( &timerBuffer + sizeof(Timer_Data_t)*Timerindex ),
								&timerBuffer_c,								
...
/*
			...
			printf(" - NF: ");
			printFloat( tptr->values->op.store_max_freq[tptr->config_sys->core_domain_id[i]]);
			printf("\n\r");
#endif
*/
...
varValue fMultiInputProcess(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args)
{
	...
	for (varFor core = i_start; core < i_end; core+=i_increment)
	{
		...
		for (varFor k = 0; k < PCF_MODEL_COEFF; k++)
		{
			...
			for (varFor j = 0; j < tptr->config_sys->num_wl_states; j++)
			{
				...
				power_formula_coeff[k] += tptr->config_sys->base_power_coeff[ j ][k] * tptr->inputs->msr.perf.perc[core][j] / VD_100_PERC;
			}
	...
	if (tptr->values->ctrl_counter % tptr->config_ctrl->therm.der_mul == 0)
	{
		for (varFor core = i_start; core < i_end; core+=i_increment)
		{
			tptr->values->ip.temp_der[core] = tptr->inputs->msr.temp.core[core] - tptr->values->ip.prev_temp[core];
...
inline void vPowerBudgetProcess(struct ctrl_task_index* tptr)
{
	if (tptr->values->ip.prev_power_budget != tptr->inputs->cmd.total_power_budget)
	{
        ...
        //if (tptr->values->ip.prev_power_budget  < tptr->inputs->cmd.total_power_budget) //TBC: only <, because this fix working good on increasing the power budget, but bad on decreasing it.
		{
...
inline void vPowerConsumptionAdaptation(struct ctrl_task_index* tptr)
{
	...
	*/
	...
	varValue total_power_adaptation_delta = tptr->inputs->msr.pw.total - tptr->values->ip.prev_tot_power[0];
	...
	if (tptr->values->ma.alpha_counter > 0)
	{
		...
		total_power_adaptation_alpha = (0.8f - total_power_adaptation_alpha) / 
		             PW_CHANGED_ALPHA_COUNTER * tptr->values->ma.alpha_counter + total_power_adaptation_alpha;
 	}

	tptr->values->ip.total_power_adaptation_term *= (VD_ONE - total_power_adaptation_alpha);
	tptr->values->ip.total_power_adaptation_term += total_power_adaptation_delta * total_power_adaptation_alpha;	
...
varValue fComputeEstimatedTotalPower (varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args)
{
    ...
    for (varFor i = i_start; i < i_end; i+=i_increment)
	{
		if (tptr->config_ctrl->conf.variable_voltage_solution == VV_MOVING_AVERAGE)
        {
            ...
            varValue freq_reduction_coeff = tptr->values->op.computed_core_frequency[i] / tptr->values->ma.og_freq[i];
...
varValue fCorePowerComputation(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args){
	...
	for (varFor i = i_start; i < i_end; i+=i_increment)
	{
		...
		for (varFor k = 0; k < PCF_MODEL_COEFF; k++)
		{
			...
			{
				//power_formula_coeff[k] += i_ctrl_parameter_table->power_formula_coeff[ j ][k] * (varValue)i_ctrl_parameter_table->perc_workload[i][j] / VD_100_PERC;
				power_formula_coeff[k] += tptr->config_sys->base_power_coeff[ j ][k] * tptr->inputs->msr.perf.perc[i][j] / VD_100_PERC;
			}
		...
		varValue core_power = lMathPowerCompute(iFreq[i], 
		...
		*/
		...
		if (core_power < tptr->config_sys->core_idle_power)
		{
			...
			printf(" - V: ");
			printFloat(tptr->values->op.computed_domain_voltage[tptr->config_sys->core_domain_id[i]]);
			printf(" - CI: ");
...
varValue fVoltageCastVote(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args)
{
    ...
    for (varFor i = i_start; i < i_end; i+=i_increment)
	{
		...
		{
			if (freq <= tptr->config_sys->voltage_table[index*3+2]) //TODO here just lloking at the min freq, not considering voltage overheads or other changes
			{
...
varValue fVoltageChoice(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args)
{
    ...
    for (varFor i = i_start; i < i_end; i+=i_increment)
	{
		...
		while (index >= 0)
		{
			accumulator += (uint32_t)i_voltage_voting[index + i*tptr->config_sys->num_voltage_levels] * 100;

			/*
			if (i==2){
			printf("Accumulator: %d, vv: %d\n\r", accumulator, i_voltage_voting[index + i*tptr->config_sys->num_voltage_levels]);
			}
			*/

			if (accumulator >= tptr->inputs->cmd.perfmormance_level * tptr->config_sys->num_cores_per_domain[i])
			{
				tptr->values->op.computed_domain_voltage[i] = tptr->config_sys->voltage_table[index*3+0]; //TODO here just lloking at the min freq, not considering voltage overheads or other changes
		...
		if (index < 0)
		{
			...
			tptr->values->op.computed_domain_voltage[i] = tptr->config_sys->voltage_table[0*3+0]; //TODO here just lloking at the min freq, not considering voltage overheads or other changes
	...
	{
		tptr->values->op.computed_domain_voltage[i] = tptr->config_sys->voltage_table[(6-1)*3+0];
...
varValue fFVpreAdaptation(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args)
{
	...
	{
		int indexD = 0;
		while ((tptr->values->ip.temp_der[i] > tptr->config_ctrl->therm.fuzzy_der_lim[indexD])&&(indexD < 4)) //TODO make it parametric
		{
			indexD++;
		}
		int indexT = 0;
		while ((tptr->inputs->msr.temp.core[i] > tptr->config_ctrl->therm.fuzzy_temp_lim[indexT]+273.15)&&(indexT < 4)) //TODO make it parametric
		{
			indexT++;
		}
		...
		/*
		if (i==2) {
		printf("Core %d: VCred: %d - fuzzy value: %d\n\r", i, tptr->values->th.fuzzy_vcred[i], tptr->config_ctrl->therm.fuzzy_value_table[indexT*5+indexD]);
		...
		*/
		...
		tptr->values->th.fuzzy_vcred[i] += tptr->config_ctrl->therm.fuzzy_value_table[indexT*5+indexD]; //TODO parametrize the 5.
...
inline void vFVdomainAdaptation(struct ctrl_task_index* tptr)
{
	...
	{
		if (tptr->values->th.fuzzy_vcred[core] < min_domain_vcred[tptr->config_sys->core_domain_id[core]])
		{
			min_domain_vcred[tptr->config_sys->core_domain_id[core]] = tptr->values->th.fuzzy_vcred[core];
		}
	}
	...
	for (varFor domain=0;domain<tptr->config_sys->num_domains;domain++)
	{
		...
		printf(" - level: %d, red: %d, new V: ", tptr->values->op.store_voltage_level[domain], min_domain_vcred[domain] / 2);
		}
		*/

		//Reduce V
		tptr->values->op.store_voltage_level[domain] += (min_domain_vcred[domain] / 2); //TODO parametrize / 2
		...
		tptr->values->op.computed_domain_voltage[domain] = tptr->config_sys->voltage_table[tptr->values->op.store_voltage_level[domain]*3+0]; 
		tptr->values->op.store_max_freq[domain] = tptr->config_sys->voltage_table[tptr->values->op.store_voltage_level[domain]*3+2];
		...
		*/
		...
		varValue F_adaptation = tptr->values->op.store_max_freq[domain] + 
			(varValue)(min_domain_vcred[domain] % 2) * 0.05; //TODO parametrize both 2 (with above) and 0.05
		...
		for (varFor core=0;core<tptr->config_sys->num_cores;core++)
		{
			if (tptr->config_sys->core_domain_id[core] == domain)
			{
				tptr->values->core_frequency[core] = 
					tptr->values->core_frequency[core] > F_adaptation ? F_adaptation : tptr->values->core_frequency[core];
...
inline void vAlphaPowerReduction(varValue i_delta_power, varValue est_total_pw, struct ctrl_task_index* tptr){
	...
	// Here we limit the iteration to an upper bound to define the maximum time, for scheduling purposes
	while ( (pw_storage < safety_value) && (iterations < 3) && (break_while < tptr->config_sys->num_cores) ) //TODO & TBD parameter 3
	{
		/*
		...
		*/
		...
		for (varFor core=0;core<tptr->config_sys->num_cores;core++)
		{
			if (min_pw_reached[core] == PCF_TRUE)
				break_while += 1;
		}
...
varValue fComputeAlpha(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args)
{
	...
	for (varFor i = i_start; i < i_end; i+=i_increment)
	{
		...
		{
			if ((tptr->config_sys->core_critical_temperature - tptr->config_ctrl->therm.pid_temp_margin - tptr->inputs->msr.temp.core[i] - safety_margin) 
					>= 0)
			{
				Alpha[i] = 1.0f / (tptr->config_sys->core_critical_temperature - tptr->inputs->msr.temp.core[i]);
			}
			else
			{
				#if (defined(PRINTF_ACTIVE) && defined(DEBUG_PRINTF))
				...
				/*
				if (delta_power*Alpha[i] > tptr->values->target_core_power[i] - tptr->config_sys->core_idle_power)
				{
					Alpha[i] = (tptr->values->target_core_power[i] - tptr->config_sys->core_idle_power) / delta_power;
				...
				*/
				...
				if (Alpha[i] <= 0)
				{
					...
					*/
					...
					Alpha[i] = tptr->config_ctrl->pw.alpha_error_base_value; //TBD: we are whitin an ERROR!!! do not put 0.01!
...
inline void vComputeAlphaBindings(struct ctrl_task_index* tptr)
{
    ...
    //     {
    //         //test_on_number
    //         if ((i_core_binding_vector[i] > (tptr->config_sys->num_cores/2)) /*|| (i_core_binding_vector[i] < 0) since it is unsigned, is never <0*/)
    //         {
    //             //TODO
    //             //ErrorMap |= BM_ERROR_NUMBERS_VALUE;
    //             #if (defined(PRINTF_ACTIVE) && defined(DEBUG_PRINTF))
    //             printf("Number issue, i_core_binding_vector>tptr->config_sys->num_cores/2, core %d: %d\n\r", i, i_core_binding_vector[i]);
    //             #endif
		...
		if (mpwr[i] != PCF_TRUE)
		{
			...
			} else if (Alpha[i] > 1.0f)
			{
				...
				*/
				...
				Alpha[i] = tptr->config_ctrl->pw.alpha_error_base_value; //TBD: we are whitin an ERROR!!! do not put 0.99!
			}
			...
			else if (tptr->values->target_core_power[i] <= tptr->config_sys->core_idle_power) //Do this after <=0 which is check on numbers. <idle is control check, values are ok!
			{
				mpwr[i] = PCF_TRUE;
				tptr->values->target_core_power[i] = tptr->config_sys->core_idle_power;
			}
...
varValue fComputeTemperatureReduction(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args)
{
	for (varFor i = i_start; i < i_end; i+=i_increment)
	{
		...
		//PID
		core_reduced_value = lMathPidCompute(tptr->values->target_core_power[i], 
		...
		//TestOnNumber
		if ( (core_reduced_value <= 0) || (core_reduced_value > tptr->values->target_core_power[i]) )
		{
			//TODO c_error_map |= BM_ERROR_NUMBERS_VALUE;
			#if (defined(PRINTF_ACTIVE) && defined(DEBUG_PRINTF))
			...
			tptr->values->target_core_power[i] = tptr->config_sys->core_idle_power; //TBD: maybe too much conservative in case > ?? (consider still there is an error!)
		}
...
void vComputeFrequency(struct ctrl_task_index* tptr)
{
	...
	//TODO
	if (tptr->config_ctrl->conf.use_freq_binding == PCF_TRUE) //TODO create a local variable. Also check the code.
	{
        //TODO
	...
	if (tptr->config_ctrl->conf.variable_voltage_solution == VV_MOVING_AVERAGE)
	{
		...
		for (varFor i = 0; i < tptr->config_sys->num_cores; i++)
		{
			...
			if (tptr->values->ma.alpha_counter > 0) //c_parameters_table.tf_changed_alpha_counter > 0)
			{
				//TODO: formula of parabola thought 2 points and and line
				//for the moment linear //TODO GLOBAL ETC
				//TODO: also maybe interpolate the line: e.g. if the 2 pw budget are very different the 0.8 is 0.95, if they are 
				...
				ma_alpha_c = (1.0f - ma_alpha_c) / 
						(TF_CHANGED_ALPHA_COUNTER) * tptr->values->ma.alpha_counter  //c_parameters_table.tf_changed_alpha_counter 
						+ ma_alpha_c;
			...
			//    (tptr->inputs->cmd.target_freq[i] - tptr->values->op.computed_core_frequency[i]) * ma_alpha_c;
			//without:
			//TBD: here tptr->values->core_frequency or tptr->inputs->cmd.target_freq?
			tptr->values->ma.freq_point[i] += ((tptr->inputs->cmd.target_freq[i] - tptr->values->op.computed_core_frequency[i])>0)?
				((tptr->inputs->cmd.target_freq[i] - tptr->values->op.computed_core_frequency[i]) * ma_alpha_c):0;
...
varValue fFrequencyCouplingTransformation(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args)
{
    ...
    //TODO: I don't think this is parallelizable? I should maybe set up something that understand which is better
	...
	for (varFor domain = i_start; domain < i_end; domain+=i_increment)
	{
		...
		func_param->num_cores = lPopulateCouplingStructure(0, tptr->config_sys->num_cores, 1, tptr, l_ptr_args);
		...
		#ifdef USE_BISECT
		#ifndef OPTIMIZATION_FUNC
        bisec(func_param->ntrial, x_solution, func_param->num_cores, func_param->tolx, func_param->tolf, l_inf, l_sup, func_param, pwrfunc); //TODO add fmin e fmax
		#else //TODO: here BE CAREFUL BECAUSE N_CORE != Xn!!!!!!!!
		bisec(func_param->ntrial, x_solution, func_param->num_cores +1, func_param->tolx, func_param->tolf, l_sup, func_param, usrfunc); //TODO add fmin e fmax
		#endif
		#else
		mnewt(func_param->ntrial, x_solution, func_param->num_cores, func_param->tolx, func_param->tolf, 0.4, 3.66, func_param, usrfunc); //TODO add fmin e fmax
		...
		{
			if (f_max <= tptr->config_sys->voltage_table[index_voltage*3+2]) //TODO here just lloking at the min freq, not considering voltage overheads or other changes
			{
				tptr->values->op.computed_domain_voltage[domain] = tptr->config_sys->voltage_table[index_voltage*3+0];
				//printf("voltage index domain %d = %d\n\r", domain, index_voltage);
                tptr->values->op.store_max_freq[domain] = tptr->config_sys->voltage_table[index_voltage*3+2];
		...
		if (index_voltage == tptr->config_sys->num_voltage_levels)
		{
			...
			//casting vote:
			tptr->values->op.computed_domain_voltage[domain] = tptr->config_sys->voltage_table[(index_voltage-1)*3+0];
            tptr->values->op.store_max_freq[domain] = tptr->config_sys->voltage_table[(index_voltage-1)*3+2];
		}
...
//varFor i_domain, struct coupling_p2f *i_func_param)
{
	...
	varValue bound_k_max = i_func_param->k_vdd_stat * 
		tptr->config_sys->voltage_table[(tptr->config_sys->num_voltage_levels-1)*3] * tptr->config_sys->voltage_table[(tptr->config_sys->num_voltage_levels-1)*3];
	varValue bound_k_min = i_func_param->k_vdd_stat * 
		tptr->config_sys->voltage_table[0] * tptr->config_sys->voltage_table[0];
	varValue bound_fv_max = tptr->config_sys->voltage_table[(tptr->config_sys->num_voltage_levels-1)*3] * 
    ...
    for (varFor core = i_start; core < i_end; core+=i_increment)
    {
        if (tptr->config_sys->core_domain_id[core] == i_domain)
        {
			...
			//TBD: here tptr->values->core_frequency or tptr->inputs->cmd.target_freq?
			i_func_param->ptr_x[counter_core]  = (tptr->values->op.computed_core_frequency[core]<tptr->values->core_frequency[core]?tptr->values->op.computed_core_frequency[core]:tptr->values->core_frequency[core]);
            ...
            //Test Boundaries:
            #ifndef USE_BISECT
            if ( (i_func_param->ptr_Pc[counter_core] - i_func_param->k_stat - bound_k_max) / i_func_param->ptr_wl[counter_core] > bound_fv_max) //TBC: is it bound_k_max or bound_k_min?
            {
                #if (defined(PRINTF_ACTIVE) && defined(DEBUG_PRINTF))
                printf("NR bound issue, max\r\n");
                #endif
                varValue future_Pc = i_func_param->ptr_wl[counter_core]*bound_fv_max + bound_k_max + i_func_param->k_stat;
                ...
                //func_param_Pc[counter_core] += func_param_Pc[counter_core] - future_Pc; ADD IN THE BIN, not HERE!
                i_func_param->ptr_Pc[counter_core] = future_Pc;
            }
            else if ( (i_func_param->ptr_Pc[counter_core] - i_func_param->k_stat - bound_k_min) / i_func_param->ptr_wl[counter_core] < bound_fv_min) //TBC: is it bound_k_max or bound_k_min?
            {
                #if (defined(PRINTF_ACTIVE) && defined(DEBUG_PRINTF))
                printf("NR bound issue, min. Pc, Total: ");
                printFloat(i_func_param->ptr_Pc[counter_core]);
                printf(" , ");
                printFloat((i_func_param->ptr_Pc[counter_core] - i_func_param->k_stat - bound_k_min) / i_func_param->ptr_wl[counter_core]);
                printf("\n\r");
                #endif

                varValue future_Pc = i_func_param->ptr_wl[counter_core]*bound_fv_min + bound_k_min + i_func_param->k_stat;
                //ifdef check on number
                if (future_Pc <= i_func_param->ptr_Pc[counter_core])
                {
                    l_error_map |= BM_ERROR_NUMBERS_VALUE;
                ...
                //func_param_Pc[counter_core] += func_param_Pc[counter_core] - future_Pc; //HERE NEGATIVE!, BC! ADD IN THE BIN, not here!
                i_func_param->ptr_Pc[counter_core] = future_Pc;
            }
            #endif //not USE_BISECT
...
void vProcessFrequency(varFor i_core, varValue i_frequency, struct ctrl_task_index* tptr)
{
    ...
    if (i_frequency > tptr->values->op.computed_core_frequency[i_core] + tptr->config_ctrl->output.freq_max_gradient)
    {
        tptr->values->op.computed_core_frequency[i_core] += tptr->config_ctrl->output.freq_max_gradient;
            ...
            (tptr->values->th.hyst_thresh_reached[i_core]) )
    {
        ...
        else
        {
            ...
            tptr->values->op.computed_core_frequency[i_core] = tptr->config_sys->core_min_frequency; //TBD:
    ...
    }else if (tptr->values->op.computed_core_frequency[i_core] > tptr->config_sys->core_max_frequency)
    {
        //TBC: is this really an error value?
        //TODO c_error_map |= BM_ERROR_NUMBERS_VALUE;
        //printf("Number issue, computed_core_frequency, core %d: %d\n\r", i, (int)(computed_core_frequency[i_core]*1000.0f));
...
varValue fFrequency2PowerConversion(varFor i_start, varFor i_end, varFor i_increment, struct ctrl_task_index* tptr, void** args)
{
    for (varFor i = i_start; i < i_end; i+=i_increment)
	{
		...
		varValue target_single_frequency = lMathFrequencyCompute(tptr->values->target_core_power[i], 
                tptr->values->op.computed_domain_voltage[tptr->config_sys->core_domain_id[i]],
                power_formula_coeff);
...
void vTelemetryManagement(void /*struct ctrl_task_index* tptr*/)
{
	...
	if (p_values_ctrl->telemetry_counter <= 0) //TBD: <= or == ? <= also take into account some errors.
	{
		...
		for (varFor i = 0; i < tptr->config_sys->num_cores; i++)
		{
			...
			c_telemetry_table.avg_core[i].freq	/= (varValue)tptr->inputs->cmd.telemetry_horizon;
			c_telemetry_table.avg_core[i].voltage 	    /= (varValue)tptr->inputs->cmd.telemetry_horizon;
			c_telemetry_table.avg_core[i].temp  /= (varValue)tptr->inputs->cmd.telemetry_horizon;

			c_telemetry_table.avg_core[i].est_pw 		    /= (varValue)tptr->inputs->cmd.telemetry_horizon;
			c_telemetry_table.chip_est_pw			    += c_telemetry_table.avg_core[i].est_pw;
		}
		//c_telemetry_table.chip_est_pw 				/= (varValue)tptr->inputs->cmd.telemetry_horizon;

		//test_on_number
        //TODO
        /*
		if ((c_telemetry_table.chip_avg_estimated_power) > (tptr->config_sys->core_max_power_single * (varValue)tptr->config_sys->num_cores) ) //TODO: EPI_CORE_IDLE_POWER
		{
			l_error_map |= BM_ERROR_NUMBERS_VALUE;
			#if (defined(PRINTF_ACTIVE) && defined(DEBUG_PRINTF))
