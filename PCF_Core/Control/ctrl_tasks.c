...
#if (MEASURE_ACTIVE == 1)
        timerBuffer[Timerindex++] = (Timer_Data_t){'p', lMeasureReadCsr(MEASURE_ZEROING)};
// printf("\n\n\rcycles: %d\n\r", timerBuffer[Timerindex-3].cycle -
// timerBuffer[Timerindex-4].cycle);
#endif
...
void vRearControlTask(void *parameters) {
     ...
     * values (initially only the EPI core VR, then also HBM, DRAM, MB, and other sockets VR
     * values); it also do an adaptive control algorithm to update the parameters that are used to
     * compute optimal voltage values (both set values and PID parameters). The task will held data
     * as workload histrory, mean power consumption, etc.
     */
    ...
    *adr_g_error_map = BM_RESET;
    ...
    for (;;) {
        ...
        // if (bReadGlobalVariable(&l_ctrl_input_table, G_CTRL_INPUT_TABLE,
        ...
        // if (bReadGlobalVariable(&l_todo_telemetry, G_TELEMETRY, sizeof(l_todo_telemetry), id) !=
        // PCF_TRUE)
        //{
        //	l_error_map |= BM_ERROR_SHARED_VAR_READ;
        ...
        */
        ...
        /*-----------------------------------------------*/
        // TONOTE (todo): qui passo già la misura della cpu, invece che del chip, ma il nome è del
        // chip
        rms_error = 0;
        // TODO: rms_error = (l_todo_telemetry.chip_avg_measured_power -
        // l_todo_telemetry.chip_avg_estimated_power);
        // test_on_number
        // todo
        ...
        */
        ...
        // void lmsCompute(ctrl_inputs_table_t *i_ctrl_parameter_table, telemetry_t*
        // i_telemetry_struct)
        //{

#ifdef LMS_ACTIVE
        // TODO: optimize all
        for (varFor j = 0; j < PCF_WL_STATES; j++) perc_wl_states[j] = 0;
        ...
        for (varFor i = 0; i < PCF_WL_STATES; i++) {
            ...
            lms_inputs[i * PCF_MODEL_COEFF + 1] =
                perc_wl_states[i] * V_Fixed * V_Fixed *
                i_telemetry_struct->core_avg_sensor_data[i].frequency;
        ...
        if (alternate_flag) {
            ...
            if (lms_batch_counter > LMS_BATCH) {
                ...
                for (int i = 0; i < PCF_WL_STATES; i++)
                    for (int j = 0; j < PCF_MODEL_COEFF; j++) {
                        if (theta2[i * PCF_MODEL_COEFF + j] >
                            cEPI_BASIC_WORKLOAD[i][j] + lms_lim[i][j])
                            theta2[i * PCF_MODEL_COEFF + j] =
                                cEPI_BASIC_WORKLOAD[i][j] + lms_lim[i][j];
                        else if (theta2[i * PCF_MODEL_COEFF + j] <
                                 cEPI_BASIC_WORKLOAD[i][j] - lms_lim[i][j])
                            theta2[i * PCF_MODEL_COEFF + j] =
                                cEPI_BASIC_WORKLOAD[i][j] - lms_lim[i][j];
                        // i_ctrl_parameter_table->power_formula_coeff[i][j] =
                        // theta2[i*PCF_MODEL_COEFF + j];
                    }
            } else
                lms_batch_counter++;
            ...
            if (lms_batch_counter > LMS_BATCH) {
                ...
                for (int i = 0; i < PCF_WL_STATES; i++)
                    for (int j = 0; j < PCF_MODEL_COEFF; j++) {
                        if (theta1[i * PCF_MODEL_COEFF + j] >
                            cEPI_BASIC_WORKLOAD[i][j] + lms_lim[i][j])
                            theta1[i * PCF_MODEL_COEFF + j] =
                                cEPI_BASIC_WORKLOAD[i][j] + lms_lim[i][j];
                        else if (theta1[i * PCF_MODEL_COEFF + j] <
                                 cEPI_BASIC_WORKLOAD[i][j] - lms_lim[i][j])
                            theta1[i * PCF_MODEL_COEFF + j] =
                                cEPI_BASIC_WORKLOAD[i][j] - lms_lim[i][j];
                        i_ctrl_parameter_table->power_formula_coeff[i][j] =
...
*/
       ...
       //}
        ...
        /*
        if (bWriteGlobalVariable(&l_ctrl_input_table, G_CTRL_PARAMETER_TABLE, GLOBAL_WRITE_ALL,
        sizeof(l_ctrl_input_table), id) != PCF_TRUE)
        {
                //TODO: do something
        }

        if (bWriteGlobalVariable(&l_todo_telemetry, G_TELEMETRY, GLOBAL_WRITE_ALL,
        sizeof(l_todo_telemetry), id) != PCF_TRUE)
        {
                //TODO: do something
        }
        */
...
void vFastControlTask(void *parameters) {
    ...
    *adr_g_error_map = BM_RESET;
    ...
    // Get initial Values
    // if (bReadGlobalVariable(&l_ctrl_values_table, G_CTRL_PARAMETER_TABLE,
    // sizeof(l_ctrl_values_table), id) != PCF_TRUE)
    /*{
            l_error_map |= BM_ERROR_SHARED_VAR_READ;
            #if (defined(PRINTF_ACTIVE) && defined(DEBUG_PRINTF))
    ...
    // TODO:
    int ratio = (g_TaskConfigTable.tasks_period_us[PERIODIC_CONTROL_TASK] /
                 g_TaskConfigTable.tasks_period_us[FAST_CONTROL_TASK]);
    ...
    for (;;) {
        ...
        if (counter_times <= 0) {
            ...
            // TBD: Do we need an extra layer (or a substitutive one) where instead of this we just
            // check a global var? (be careful with races)
            // l_periodicity_counter = g_TaskConfigTable.tasks_tap_multiplier[REAR_CONTROL_TASK];
            // //TBD: if we use global variable check, this should be put elsewhere.
            ...
            for (varFor core = 0; core < g_config_sys.num_cores; core+=1)
            {
                    ...
                    {
                            if (freq <= g_config_sys.voltage_table[index*3+2]) //TODO here just
            lloking at the min freq, not considering voltage overheads or other changes
                            {
            ...
            {
                    if (g_config_sys.voltage_table[voltage_checking[domain]*3+0] <
            g_shared_task.op.computed_domain_voltage[domain])
                    {
                            /*
            ...
            /*
            g_shared_task.op.computed_domain_voltage[domain] =
g_config_sys.voltage_table[voltage_checking[domain]*3+0];
    }
        ...
        if (scmi_target_changed && SCMI_IMPROV) {
            ...
            if (freq < g_shared_task.op.computed_core_frequency[SCMI_CORE]) {
                ...
                */
                ...
                // TODO: HERE I SHOULD CHECK VARIOUS THINGS, I'M JUST ASSUMING 1 DOMAIN PER CORE,
                // THIS IS DONE
                //		JUST FOR THE PAPER!!!!!!!!!
                g_shared_task.op.computed_core_frequency[SCMI_CORE] = freq;
                fast_freq[2] = freq;
                // TODO: ALSO I'M NOT CONSIDERING PLL TIME TO CHANGE, AND I'M NOT CONSIDERING THE
                // FACT THAT I SHOULD LOWER THE FREQUENCY THAN WAIT FOR THE VOLTAGE!
                bSecureSendCoreFrequencies(g_shared_task.op.computed_core_frequency);
                varFor index = 0;
                while (index < g_config_sys.num_voltage_levels) {
                    if (freq <= g_config_sys.voltage_table[index * 3 + 2]) // TODO here just lloking
                                                                           // at the min freq, not
                                                                           // considering voltage
                                                                           // overheads or other
                                                                           // changes
...
// printf("Error voltage choice core %d > max\n\r", i);
                    ...
                    // casting vote:
                    voltage_checking[index - 1 +
                                     SCMI_CORE * g_config_sys.core_domain_id[SCMI_CORE]]++;
                }

                if (g_config_sys.voltage_table[voltage_checking[SCMI_CORE] * 3 + 0] <
                    g_shared_task.op.computed_domain_voltage[SCMI_CORE]) {
                    /*
                    ...
                    */
                    ...
                    g_shared_task.op.computed_domain_voltage[SCMI_CORE] =
                        g_config_sys.voltage_table[voltage_checking[SCMI_CORE] * 3 + 0];
                }

                if (bSecureSendDomainVoltages(g_shared_task.op.computed_domain_voltage) !=
                    PCF_TRUE) {
                    // TODO
