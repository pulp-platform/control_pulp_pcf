...
varValue lMathPidCompute(varValue i_target_power, varValue i_measured_temperature,
                         varFor i_core_number, struct ctrl_thermal *th,
    ...
    */
    ...
    /*
            if ((i_measured_temperature> EPI_CORE_CRITICAL_TEMPERATURE - 50) && (i_core_number ==
       0))
            {
                    printf("asd  ");
    ...
    if (config_ctrl->conf.use_pid_integral_action == PCF_TRUE) {
        ...
        */
        ...
        if (config_ctrl->conf.use_pid_anti_windup_sat == PCF_TRUE) {
            if (th->pid_integral_error[i_core_number] >
                config_ctrl->therm.pid_anti_windup_sat_up) // TBD: Should I parametrize this 0?
                th->pid_integral_error[i_core_number] = config_ctrl->therm.pid_anti_windup_sat_up;
            else if (th->pid_integral_error[i_core_number] <
                     (config_ctrl->therm.pid_anti_windup_sat_coeff * i_target_power)) // TODO: var
                                                                                      // conversion
            {
                th->pid_integral_error[i_core_number] =
                    config_ctrl->therm.pid_anti_windup_sat_coeff *
                    i_target_power; // TODO: var conversion
            }
        }
    ...
    /*
            if ((i_measured_temperature> EPI_CORE_CRITICAL_TEMPERATURE - 6.2) && (i_core_number ==
       0))
            {
                    printf("asd  ");
    ...
    */
    ...
    if ( (output_pid <= 0) || (output_pid > i_target_power) )
    {
            ...
            output_pid = EPI_CORE_IDLE_POWER; //TBD: maybe too much conservative in case > ??
...
varBool_e bMathPidSetParameters(struct ctrl_thermal *therm_table) { // TBD: better to pass by copy
                                                                    // or by reference?
    ...
    therm_table->Tcrit_pid =
        g_config_sys.core_critical_temperature - g_ControlConfigTable.therm.pid_temp_margin;
...
varValue lMathPowerCompute(varValue i_target_frequency, varValue i_target_voltage,
                           varValue *i_formula_coeff) { // TBD: better to pass by copy or by
                                                        // reference? (same for output)

    /* Formula: P = K1*f*V^2 + K2 + (K3 + K4*f)*w^K5 */

    // return (i_formula_coeff[0] * i_target_frequency * (V_Fixed*V_Fixed) ) + i_formula_coeff[1] +
    //		( (i_formula_coeff[2] + (i_formula_coeff[3] * i_target_frequency)) * iWorkload );
    ////TODO: Formula miss k5
    // Same formula as Christian. checked.

    // printf("pc: " );
    // printFloat((Icc + (iWorkload * (i_target_frequency /*/ 1000000000.0*/) * V_Fixed) ) * V_Fixed
    // ); return ( (Icc + (iWorkload * (i_target_frequency /*/ 1000000000.0*/) * V_Fixed) ) *
    // V_Fixed );

    // printFloat(i_formula_coeff[0]);
    // printFloat(i_formula_coeff[1]);
    // printFloat((i_formula_coeff[0] + (i_formula_coeff[1] * (i_target_frequency /*/
    // 1000000000.0*/) * V_Fixed) ) * V_Fixed );

#ifdef USE_RHEA
    return ((i_formula_coeff[0] * i_target_voltage +
             (i_formula_coeff[1] * (i_target_frequency /*/ 1000000000.0*/))) *
                i_target_voltage +
            0.469f);
#elif defined(USE_MONTE_CIMONE)
    return ((i_formula_coeff[0] + (i_formula_coeff[1] / 0.9 / 0.9 *
                                   (i_target_frequency /*/ 1000000000.0*/) * i_target_voltage)) *
                i_target_voltage +
            0.52804f);
#else // Legacy, HIPEAC
    return ((i_formula_coeff[0] +
             (i_formula_coeff[1] * (i_target_frequency /*/ 1000000000.0*/) * i_target_voltage)) *
            i_target_voltage);
#endif
...
varValue lMathFrequencyCompute(varValue i_core_target_power, varValue i_domain_target_voltage,
                               varValue *i_formula_coeff) {

    // TBC: Check formula. Miss (^k5) in the end
    /*return (i_core_target_power - i_formula_coeff[1] - i_formula_coeff[2] * iWorkload) /
                                                            (V_Fixed * V_Fixed * i_formula_coeff[0]
       + i_formula_coeff[3] * iWorkload);
    */

#if ((CONTROL_DATA_TYPE == 1) || (CONTROL_DATA_TYPE == 2))
    // printf("ciaddo\n\r" );
    // return ( ((i_core_target_power - (Icc*V_Fixed)) / iWorkload / (V_Fixed*V_Fixed)) /*
    // *1000000000.0*/);
    varValue step = 10.0f;

#ifdef USE_RHEA
    varValue frequency = (((i_core_target_power - 0.469f) / i_target_voltage) -
                          (i_formula_coeff[0] * i_target_voltage)) /
                         i_formula_coeff[1];
#elif defined(USE_MONTE_CIMONE)
    varValue frequency =
        ((((i_core_target_power - 0.52804f) / i_domain_target_voltage) - i_formula_coeff[0]) /
         i_domain_target_voltage / i_formula_coeff[1] * 0.9 * 0.9);
#else // Legacy, HIPEAC
    varValue frequency = ((i_core_target_power / i_domain_target_voltage - i_formula_coeff[0]) /
                          i_formula_coeff[1] / i_domain_target_voltage);
#endif
    ...
    // varValue frequency = ( ((i_core_target_power - (i_formula_coeff[0]*V_Fixed)) /
    // i_formula_coeff[1] / (V_Fixed*V_Fixed)));
    varValue frequency =
        (((i_core_target_power - (i_formula_coeff[0] * i_domain_target_voltage)) /
          i_formula_coeff[1] / (i_domain_target_voltage * i_domain_target_voltage)));
...
varValue lMathLmsRecursive(varValue *o_param, varValue *o_Pcurr, varValue *i_prev_param,
                           varValue *i_Pprev, varValue *i_input_value, varValue i_error,
                           varValue i_lambda) {

    // p_est = p_est(prev) + K*error;
    // error = measured_power - power_computed_with_prev_paramenters;
    // k = P*[V, VVF]
    // P = P(prev)/lambda/(t-1) - P(prev)*[V, VVF]*[V, VVF]T*P(prev)/(t-1)/(lambda*(lambda*(t-1) +
    // [V, VVF]T*P(prev)*[V, VVF])))
    ...
    MatMul(T3, T2, i_input_value, LMS_CN, 1, LMS_CN); // since the vector is Nx1, no need to do the
                                                      // Transpose since the Matrix is implemented
                                                      // as an array
    MatMul(T4, T3, i_Pprev, LMS_CN, LMS_CN, LMS_CN);

    // is a number, maybe is worth to create its own function
    // MatMul(coeff2, i_input_value, T2, 1, iN, 1); //since the vector is Nx1, no need to do the
    // Transpose since the Matrix is implemented as an array
...
void vMathLmsStoreP(varValue *o_param, varValue *i_input_value) {
    ...
    for (int i = 0; i < PCF_MODEL_COEFF * PCF_MODEL_COEFF; i++) o_param[i] += T2[i];
...
*/
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (ii)
            ...
            ii = i + 1; // A nonzero element was encountered, so from now on we will have to do the
...
void mnewt_parallel(void *args) {
    ...
    unsigned int dma_id_temp =
        pulp_cl_idma_memcpy(x_init, casted_args->x, sizeof(float) * PCF_CORES_MAX);
    plp_cl_dma_wait(dma_id_temp);
    dma_id_temp =
        pulp_cl_idma_memcpy(func_param_wl, casted_param->ptr_wl, sizeof(float) * PCF_CORES_MAX);
    plp_cl_dma_wait(dma_id_temp);
    dma_id_temp =
        pulp_cl_idma_memcpy(func_param_Pc, casted_param->ptr_Pc, sizeof(float) * PCF_CORES_MAX);
    plp_cl_dma_wait(dma_id_temp);
    ...
    mnewt(casted_args->ntrial, x_init, casted_args->n, casted_args->tolx, casted_args->tolf,
          &L1_param, casted_args->usrfun);

    dma_id_temp = pulp_cl_idma_memcpy(casted_args->x, x_init, sizeof(float) * PCF_CORES_MAX);
    plp_cl_dma_wait(dma_id_temp);
}
#endif // GSL_PARALLEL

void mnewt(int ntrial, float x[], int n, float tolx, float tolf, float lim_inf, float lim_sup,
           void *param, void (*usrfun)(float *, int, float *, float (*)[PCF_CORES_MAX], void *))
/*
...
*/
{
    ...
    for (k = 0; k < ntrial; k++) {
        usrfun(x, n, fvec, fjac, param); // User function supplies function values at x in fvec and
                                         // Jacobian matrix in fjac.
        errf = 0.0;
        ...
        for (i = 0; i < n; i++) // Update solution.
        {
            ...
            if (value1 < 0) {
                ...
                if (value1_app < 0) value1_app = -value1_app;
                if (value1_app > divergence_factor) divergence_factor = value1;

            } else if (value2 > 0) {
                diverged = 1;
                if (value2 > divergence_factor) divergence_factor = value2;
            }
        }
#else  // GSL_PARALLEL
        float inv_fjac[PCF_CORES_MAX][PCF_CORES_MAX];
        struct coupling_p2f *param_casted = (struct coupling_p2f *)param;
        plp_mat_inv_instance_f32 args = {.pSrc = fjac,
                                         .pDst = inv_fjac,
                                         .flag = 0U,
                                         .N = param_casted->num_cores,
        ...
        plp_mat_mult_instance_f32 args2 = {.pSrcA = inv_fjac,
                                           .pSrcB = p,
                                           .M = param_casted->num_cores,
                                           .N = param_casted->num_cores,
                                           .O = 1,
                                           .nPE = pi_cl_cluster_nb_cores(),
                                           .pDstC = fvec};
        ...
        for (i = 0; i < n; i++) // Update solution.
        {
            ...
            if (value1 < 0) {
                diverged = 1;
                if (value1 > divergence_factor) divergence_factor = value1;

            } else if (value2 > 0) {
                diverged = 1;
                varValue value2_app = value2;
                if (value2_app < 0) value2_app = -value2_app;
                if (value2_app > divergence_factor) divergence_factor = value2;
            }
        }
#endif // GSL_PARALLEL
...
void bisec(int ntrial, varValue x[], int n, varValue tolx, varValue tolf, varValue *lim_inf,
           varValue *lim_sup, void *param,
...
*/
{
    ...
    for (k = 0; k < ntrial; k++) {
        ...
        for (i = 0; i < n; i++) {
            x[i] = (l_sup[i] + l_inf[i]) / 2; /*printf("X[%d] = %f\n\r", i, x[i]);*/
        }
        // Right-hand side of linear equations.
        usrfun(x, n, fvec, NULL, param); // User function supplies function values at x in fvec and
                                         // Jacobian matrix in fjac.
        ...
        for (i = 0; i < n; i++) {
            ...
            if (sign_n == sign_o) {
                l_inf[i] = x[i];
                saved_fvec[i] = fvec[i];
            } // TODO: improve this to remove saved_fvec[i]=, and instead alternate between the teo
            // l_sup[i] = x[i];
            else
                l_sup[i] = x[i];
...
fastpow2 (float p)
{
  ...
  union { uint32_t i; float f; } v = { (uint32_t)( (1 << 23) * (clipp + 121.2740575f + 27.7280233f /
...
varValue der_softmax_square(varValue *x, varValue xj, varValue alpha, varFor n) {
    ...
    varValue res = 2 * softmax * (varValue)fastexp(alpha * xj) / sumexp * // TODO: expf? o exp?
                   (1 + alpha * (xj - softmax));
...
void pwrfunc(float *F, int n, float *f, float (*J)[PCF_CORES_MAX], void *p) {
    ...
    for (varFor i = 0; i < num_cores; i++) {
        ...
        f[i] = (param->ptr_wl[i] * F[i] / 0.9f / 0.9f *
                (maxF * param->alpha_vdd + param->aoffset_vdd) *
                (maxF * param->alpha_vdd + param->aoffset_vdd)) +
               (param->k_vdd_stat * (maxF * param->alpha_vdd + param->aoffset_vdd)) +
               param->k_stat - param->ptr_Pc[i];
    }
...
void usrfunc(float *F, int n, float *f, float (*J)[PCF_CORES_MAX], void *p) {
    ...
    for (varFor i = 0; i < num_cores; i++) {
        f[i] = ki[i] * softmax_square_mem + (param->k_vdd_stat * param->alpha_vdd * softmax_mem) +
               param->k_stat - param->ptr_Pc[i];
    }
    for (varFor i = num_cores; i < PCF_CORES_MAX; i++) {
        f[i] = 0; // x = 0;
        ...
        for (varFor i = 0; i < num_cores; i++) {
            ...
            for (varFor j = 0; j < PCF_CORES_MAX; j++) {
                if (i == j) {
                    ...
                    J[i][j] = param->ptr_wl[i] * alpha_square_vdd * softmax_square_mem +
                              //	param->ptr_wl[i]*alpha_square_vdd * F[i] *
                              // der_softmax_square(F, F[j], alpha_softmax, num_cores) );
                    ...
                    J[i][j] =
                        //	param->ptr_wl[i]*alpha_square_vdd * F[i] * der_softmax_square(F,
                        // F[j], alpha_softmax, num_cores) );
                        ki[i] * der_softmax_square_mem[j];
...
void usrfunc(float *F, int n, float *f, float (*J)[PCF_CORES_MAX], void *p) {
    ...
    for (varFor i = 0; i < num_cores; i++) {
        ...
        f[i] = (param->ptr_wl[i] / 0.9f / 0.9f * (diff < 0 ? F[i] : Fv) * alpha_square_vdd * Fv) +
               (param->k_vdd_stat * param->alpha_vdd * Fv) + param->k_stat - param->ptr_Pc[i];
