...
varBool_e bImpSendCoreFreq(varValue *i_computed_freq) {
    ...
    if (no_io_periodicCounter > NO_IO_PERIODIC_STEPS_WAIT) {
        ...
        // printf("periodic counter: %d\n\r", no_io_periodicCounter);
        ...
        for (int i = 0; i < l_num_core; i++) {
            ...
            no_io_freqDiff[no_io_periodicIndex][i] =
                no_io_recordedFreqOut[no_io_periodicIndex][i] - i_computed_freq[i];
#endif
        }
    }
...
varBool_e bImpReadTempRequest(varValue *o_measured_temp) {
    ...
    if (no_io_periodicCounter > NO_IO_PERIODIC_STEPS_WAIT) {
        ...
        for (varFor i = 0; i < l_num_core; i++)
            o_measured_temp[i] = 273.15f + no_io_measuredTemp[no_io_periodicIndex][i];
...
varBool_e bImpReadInputParameters(ctrl_inputs_table_t *i_input_table) {
    ...
    // o_telemetry->chip_avg_measured_power = no_io_simPower[no_io_periodicIndex]; //[no_io_uiIndex
    // * NO_IO_UI_STEPS + no_io_uiCounter];

    // no_io_uiFreqRedMap[no_io_uiIndex * NO_IO_UI_STEPS + no_io_uiCounter] =
    // o_telemetry->frequency_reduction_map[0];
